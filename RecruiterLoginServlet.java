...
public class RecruiterLoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		getServletContext().getRequestDispatcher("/recruit/userdetails.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		UserDetails details = new UserDao().login(request.getParameterValues("emailAddress")[0], request.getParameterValues("password")[0], "recruiter");
		request.setAttribute("data", details);
		if(details== null) {
			response.sendRedirect("/OnlineJobPortal/recruit/login.html");
		}else {
			response.sendRedirect("/OnlineJobPortal/recruiterhome.jsp?id="+details.getId());
		}
	}
}
